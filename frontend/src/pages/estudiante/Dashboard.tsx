...
import { obtenerDashboardEstudiante } from "../../services/estudianteDashboard.service";
import type { EstudianteDashboard, ReservaEstudiante } from "../../types/estudianteDashboard.types";


export default function Dashboard() {

    const [dashboard, setDashboard] = useState<EstudianteDashboard | null>(null);
    ...
    useEffect(() => {
        const cargar = async () => {
            ...
            try {
                ...
                console.log("DASHBOARD ESTUDIANTE:", data);
                setDashboard(data);
            } catch (error) {
                console.error("Error cargando dashboard del estudiante:", error);
            } finally {
                setCargando(false);
            }
        };
                        ...
                        <div className="dashboard-reservation-list">
                            {reservas.map(
                                (
                                    ...
                                    const fecha = obtenerFecha(item.inicio);
                                    const horaInicio = item.inicio?.split(" ")[1]?.substring(0, 5) ?? "";
                                    const horaFin = item.fin?.split(" ")[1]?.substring(0, 5) ?? "";
                                    return (
                                        <article key={item.id} className="dashboard-reservation">
                                            {/* FECHA */}
